...
function _onload(){
	...
	/*$.ajax({
		...
		if(action.page_num=="4"){
			if(action.sub_page_flag=="true"){
				if(action.sub_page_num=="1"){
					//蹴鞠新闻
					clearTimeout(time_id);
					$("#pinglun_div").css("display","none");
					$("#pinglun_ul").css("display","none");
    					...
    					$.ajax({
    						...
    						success : function(msg){
    							if(msg){
    								var number;
    								try{
    									number = Number(newsId);
    								}catch(e){
    									number = 0;
    								}
    								$(".comment-title").html("评论区("+(msg.length*10+number)+")");
    								var tx="";
 		...
 		success : function(msg){
 			if(msg){
 				...
 				for(var i=0;i<msg.length;i++){
 					...
 					var tx="";
 					tx="<div class='getHeight'><h1 class=\"title\">"+(msg[i]["FM_NEWS_TITLE"]==null?"":msg[i]["FM_NEWS_TITLE"])+" <em>来源：蹴鞠足球</em><span class=\"fr time\">"+(msg[i]["FM_PUBLISH_DATE"]==null?"":msg[i]["FM_PUBLISH_DATE"])+"</span><span class=\"fr font24\">"+(msg[i]["_fm_news__FM_NEWS_AREA_"]==null?"":msg[i]["_fm_news__FM_NEWS_AREA_"])+"</span></h1>"+
 						" <div class=\"vd-box\">"+
 						"     <div class=\"list_lh\">"+
 						"       <ul>"+
 						"           <li>"+
 			     ...
 			     }).delay(5000).fadeOut(2500, function(){
 			    	showcjcomment(newsId,province,hotNewsChart);
 			     });
 			}
 		},
 		error : function(){
